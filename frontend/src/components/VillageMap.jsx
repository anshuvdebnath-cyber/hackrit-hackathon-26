...
export default function VillageMap({ villages, selectedVillageId, onSelect }) {
  ...
  return (
    <div className="relative isolate z-0 w-full h-[520px] rounded-2xl overflow-hidden border border-earth-300/80 shadow-lg bg-earth-200">
      ...
      <div className="absolute top-3 left-3 z-[400] bg-earth-900/90 backdrop-blur-sm text-earth-100 px-3.5 py-2 rounded-xl border border-earth-700/80 shadow-md flex items-center gap-3 text-xs pointer-events-auto">
        <div className="flex items-center gap-1.5 font-semibold text-terracotta-300">
        ...
        <span className="text-earth-600">|</span>
        <span className="text-earth-400 text-[11px] hidden sm:inline-flex items-center gap-1">
          <Crosshair className="w-3 h-3 text-terracotta-400" />
          Click map to evaluate custom coordinates
        </span>
        {customPin && (
          <button
            onClick={() => setCustomPin(null)}
            className="ml-1 text-[10px] bg-earth-800 hover:bg-earth-700 text-terracotta-300 px-2 py-0.5 rounded border border-earth-700 transition"
          >
            Clear Pin
          </button>
        )}
      ...
      {/* Map Legend Overlay */}
      <div className="absolute bottom-4 left-4 z-[400] bg-earth-900/92 backdrop-blur-md text-earth-100 p-3 rounded-xl border border-earth-700/80 shadow-lg text-xs space-y-1.5 max-w-[230px] pointer-events-auto select-none">
        <div className="font-semibold text-earth-200 mb-1 flex items-center justify-between gap-3">
          <span>Avalanche Threat Key</span>
          <span className="text-[10px] text-earth-400 font-normal">HiAVAL Standards</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="w-3 h-3 rounded-full bg-clay-500 inline-block shadow"></span>
          <span className="text-earth-200">High Risk (&gt;70)</span>
          <span className="ml-auto text-clay-300 font-mono text-[11px]">
            {villages.filter(v => v.avalancheRisk?.level === 'High').length}
          </span>
        </div>
        <div className="flex items-center gap-2">
          <span className="w-3 h-3 rounded-full bg-terracotta-500 inline-block shadow"></span>
          <span className="text-earth-200">Moderate Risk (41-70)</span>
          <span className="ml-auto text-terracotta-300 font-mono text-[11px]">
            {villages.filter(v => v.avalancheRisk?.level === 'Moderate').length}
          </span>
        </div>
        <div className="flex items-center gap-2">
          <span className="w-3 h-3 rounded-full bg-moss-500 inline-block shadow"></span>
          <span className="text-earth-200">Low Risk (&le;40)</span>
          <span className="ml-auto text-moss-300 font-mono text-[11px]">
              ...
              <div className="p-1.5 min-w-[240px] max-w-[280px]">
                {customPin.loading ? (
                  <div className="py-4 px-2 text-center space-y-2">
                    ...
                    <p className="font-semibold text-xs text-earth-800">
                  ...
                  <div>
                    <div className="flex items-start justify-between gap-2 border-b border-earth-200 pb-2 mb-2">
                      <div>
                        <div className="flex items-center gap-1 text-[10px] font-bold text-terracotta-600 uppercase tracking-wider">
                          <Sparkles className="w-3 h-3" />
                          <span>Custom Geolocation</span>
                        </div>
                        <h4 className="font-serif font-bold text-earth-900 text-sm leading-tight">
                          Analyzed Sector
                        </h4>
                        <p className="text-[10px] font-mono text-earth-500">
                          {customPin.lat.toFixed(4)}°N, {customPin.lng.toFixed(4)}°E
                        </p>
                      </div>
                      <span
                        style={{ backgroundColor: getRiskColor(customPin.data.avalancheRisk?.level) }}
                        className="text-white text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider shadow-sm"
                      >
                    ...
                    <div className="grid grid-cols-2 gap-1.5 text-xs text-earth-800 mb-2.5">
                      ...
                      <div className="bg-earth-100/90 p-1.5 rounded">
                        ...
                        <span className="font-bold" style={{ color: getRiskColor(customPin.data.avalancheRisk?.level) }}>
                    ...
                    {customPin.data.explanation && (
                      <p className="text-[11px] text-earth-700 bg-earth-50 p-1.5 rounded border border-earth-200/80 mb-2 leading-tight">
                        {customPin.data.explanation}
                      </p>
                    )}
                    ...
                    <button
                      onClick={() => setCustomPin(null)}
                      className="w-full py-1 px-2 bg-earth-200 hover:bg-earth-300 text-earth-700 rounded text-xs font-semibold flex items-center justify-center gap-1 transition"
                    >
                ...
                <div className="p-1 max-w-[240px]">
                  <div className="flex items-start justify-between gap-2 border-b border-earth-200 pb-2 mb-2">
                    <div>
                      ...
                      <p className="text-[11px] text-earth-600">{v.region}</p>
                    </div>
                    <span
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs text-earth-800 mb-3">
                    ...
                    <div className="bg-earth-100/80 p-1.5 rounded">
                      ...
                      <span className="font-bold">{v.elevation}m</span>
                    </div>
                    <div className="bg-earth-100/80 p-1.5 rounded">
                    ...
                    <div className="bg-earth-100/80 p-1.5 rounded">
                      ...
                      <span className="font-bold" style={{ color: getRiskColor(level) }}>
                  ...
                  <button
                    ...
                    className="w-full py-1.5 px-2 bg-terracotta-500 hover:bg-terracotta-600 text-white rounded-lg text-xs font-semibold flex items-center justify-center gap-1 transition-colors shadow-sm"
                  >
